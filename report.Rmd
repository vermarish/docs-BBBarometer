...
tidbits <- read_csv("data/sporadic.csv")  # this one has problems for some reason
```
...
Let's write some code to measure these quantities.

We'll compute PPV and TPR for a bunch of different thresholds.
```{r}
dt = 4e7
margin = 5*dt
...
for (threshold in seq(0.05, 0.98, 0.01)) {
  ...
  all_events <- all_events %>% 
    mutate(set = sapply(time, 
                        function(t) {if (t > cutoff) "testing" else "training"})) %>%
    filter(set == "testing")
...
performance_by_threshold <- performance_by_threshold %>%
  ...
  mutate(prod=PPV*TPR, 
         ...
         F2=5*PPV*TPR/(2*PPV+TPR))

# Identify the best threshold value
best_threshold <- performance_by_threshold %>% 
  mutate(metric = F1) %>% # CHANGE THIS TO norm OR prod
  arrange(desc(metric)) %>% 
  slice(1) %>% 
  pull(threshold)
...
performance_by_threshold %>%
  pivot_longer(cols = c(PPV, TPR, F1, Fhalf, F2), names_to = "metric") %>%
  ggplot(aes(x=threshold, y=value, color=metric)) +
  geom_line(size=1) +
  geom_point() +
  scale_y_continuous(limits=c(0,1)) +
  # set 1 as the upper limit, round the lower limit to tenths-place
  scale_x_continuous(limits=function(xlims) { c(floor(xlims[1]*10)/10, 1) }) +
  scale_color_manual(breaks = c("PPV", "TPR", "F1", "Fhalf", "F2"), 
                     values = c("#F8766D", "#619CFF", "#BBBBBB", "#EE9999", "#99EE99"),
                     labels = c("PPV (reliability, precision)", 
                                "TPR (sensitivity, recall)", 
                                "F-1 score",
                                "F-0.5 score",
                                "F-2 score")) +
  geom_vline(xintercept=best_threshold, linetype=2, color="#BBBBBB")

plot1 <- performance_by_threshold %>%
  mutate(P=TP+FP) %>%
  ggplot(aes(x=TPR, y=PPV,
             # dummy aesthetics for tooltip
             threshold=threshold, FP=FP, TP=TP, FN=FN
             )) +
...
plot2 <- performance_by_threshold %>%
  mutate(PPV=round(PPV,2), TPR=round(TPR,2)) %>%
  ggplot(aes(y=threshold, x=TPR, color=PPV,
             TP=TP, FP=FP)) +
  geom_point(size=2) +
...
plot3 <- performance_by_threshold %>%
  ggplot(aes(x=TPR, y=threshold, fill=PPV,
             # dummy aesthetics, for tooltip
             TP=TP, FP=FP)) +
  geom_col(orientation="y") +
  scale_fill_continuous(type="viridis") +
  ggtitle("True Positive Rate decreases as threshold increases")
ggplotly(plot3)
```
