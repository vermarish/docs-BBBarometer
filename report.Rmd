...
```{r functions for viewing, include=FALSE}
...
scatterplot <- function(data, sensor_type, col, 
  ...
  graph <- time_series %>%
    ...
    geom_point(colour = "#1a9850") + 
    scale_y_continuous()
  
  if (!stem) {
    graph <- graph + 
      geom_line(colour = "#1a9850")  # plot signal using line
  } else {
    graph <- graph + 
      geom_segment(aes(xend=time, yend=0), color = "#1a9850")
  }
  ...
  tap_times <- taps$time
  graph <- graph +
    geom_vline(aes(xintercept=time), data=taps, 
               colour="#ED1C24", size=size, alpha=alpha)
  ...
  guesses <- data %>%
    ...
    trim(start, end)
  graph <- graph +
    geom_vline(aes(xintercept=time), data=guesses, 
               colour="#00FF00", size=size, alpha=alpha)
  
  return(graph)
}
scatterplot(data, "pressure", "one", 0.75, 0.85, derivs=1, stem=TRUE)
```
 
## Phase One: finding touch occurrence from pressure data
...
```{r}
scatterplot(data, "pressure", "one", 0.5, 0.55, derivs=0)
scatterplot(data, "pressure", "one", 0.5, 0.55, derivs=1)
scatterplot(data, "pressure", "one", 0.5, 0.55, derivs=2)
```
...
#### wait this is dumb
Fast forward 1 year in the future. I'm a bit better with tidyverse now. Which derivative of pressure data do we really want to use? We could even use multiple! Think about it; the first derivative centers the data around 0 so we may look for positive values, then negative values. The second derivative may look for rates of change along here, which might be important, ya know?
```{r}
# Let's take repeated derivatives of pressure and see how they are impacted by touch input.
data_d0 <- data
data_d1 <- data_d0 %>%
  filter(type == "pressure") %>%
  mutate(one = c(0, diff(one))) %>%
...
scatterplot(data_d0, "pressure", "one", start=a, end=b)
scatterplot(data, "gyroscope", "one", start=a, end=b, stem=TRUE)
scatterplot(data_d1, "pressure", "one", start=a, end=b, stem=TRUE)
scatterplot(data_d2, "pressure", "one", start=a, end=b, stem=TRUE)
```

### Prep data

Okay so let's do it. Let's use the first 60\% of the data as training data, and the last 40\% as testing data. With 161 data points in experiment 1, this is 101 data points in training and 69 data points in testing.
