...
scatterplot(data, "pressure", "one", 0.7, 0.8, derivs=1)
scatterplot(data, "pressure", "one", 0.7, 0.8, derivs=2)
```
...
dt = 4e7
margin = 5*dt
# dt = 4e7 for pressure sensor
# we'll use a margin of, what, 5 dt's ?

# touch event: annotate either detected or undetected  (true_positive or false_negative)
# touch_predict event: annotate either true or false   (true_positive or false_positive)
...
Cool! Let's compute PPV and TPR for a bunch of different thresholds.
```{r}
dt = 4e7
margin = 5*dt
...
for (threshold in seq(0.05, 0.6, 0.005)) {
  thresholded <- threshold_pressure(touch_confidence, threshold=threshold) %>%
    bind_rows(data %>% filter(type=="touch"))
