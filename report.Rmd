...
mine_features <- function(data, touch_confidence, times) {
  ...
  pressure_one <- pull_waveforms(signal=touch_confidence %>% select(time=time, data=p),
                                 times=pressure_times)
  
  extremum_p <- pressure_one %>% sapply(function(waveform) {sign(waveform[[1]][1]) * max(abs(waveform[[1]]))})
...
# For testing. 
# TODO Remove when the function works, and refactor
...
build_experiment <- function(tidbits=NULL, path="data/experiment_1.csv") {
  ...
  # mine features
  exp_data <- build_df_touch(model, labeled=TRUE)
