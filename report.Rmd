...
exp3_data <- build_experiment(path="data/experiment_3.csv") %>%
  mutate(quantile=group_by_quantile(x+y))

# The experiment is distributed in an arc
exp3_map <- exp3_data %>% 
  ggplot(aes(x=x, y=y, color=quantile)) +
  geom_point() +
  theme(aspect.ratio=2400/1080) +      ##formatting
  scale_y_reverse(limits=c(2400,0)) +  ##
  scale_x_continuous(limits=c(0,1080), ##
...
exp3_matrix <- ggpairs(exp3_data, 
        columns=c(4:6,8),
        mapping = ggplot2::aes(color=quantile))

bounds <- exp3_data[4:6] %>% abs() %>% max()

exp3_space_all <- exp3_data %>%
  #filter(quantile %in% c(1,4)) %>%
  rename(side=quantile) %>%
  ggplot(aes(x=extremum_one, y=extremum_two, color=side)) +
  geom_point() +
  geom_hline(yintercept=0, color="#aaaaaa") + 
  geom_vline(xintercept=0, color="#aaaaaa") + 
  xlim(-bounds, bounds) +
  ylim(-bounds, bounds) +
  ggtitle("Experiment 3 seems cloudy at first")

exp3_space_filter <- exp3_data %>%
  filter(quantile %in% c(1,3)) %>%
  rename(side=quantile) %>%
  ggplot(aes(x=extremum_one, y=extremum_two, color=side)) +
  geom_point() +
  geom_hline(yintercept=0, color="#aaaaaa") + 
  ...
  xlim(-bounds, bounds) +
  ylim(-bounds, bounds) +
  ggtitle("Experiment 3 is not discernible with gyro")
