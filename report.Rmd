...
Great. It only took 300+ lines of code to get here, from the data presented by the Android app to an actual workable tidy learnable format. I'm going to encapsulate this in one function.
...
exp1_dupl <- build_experiment(path="data/experiment_1.csv")
```

Alright. Experiment one. Can I discern left/right movement?

```{r}
exp1_data <- gyro_from_touch(model, labeled=TRUE)

exp1_data %>% 
  ggplot(aes(x=x, y=y)) +
  geom_point() +
  xlim(0, 1080) + ylim(0, 2400) + theme(aspect.ratio=2400/1080)

model.1 <- lm(formula=x~extremum_one + extremum_two + extremum_three, 
   data=exp1_data)
model.1
...
I'm running into a minor analytic problem here. This graph shows a strong association between touch data and the gyroscope data. This is great! Decision tree classification would work very well on x. I thought it would be as simple as linear regression, but regression will be nonlinear. 