...
plot_gyro_1 <- scatterplot(exp1$data, "gyroscope", "one", start=0, end=1) +
  ...
  theme(axis.title.x=element_blank(),
        ...
        title=element_blank()) +
  geom_point(aes(x=time, y=diff),
             data=exp1$data %>%
               filter(type=="gyroscope") %>%
               mutate(diff = lead(one,2)-lag(one,2)),
             color="#9999DD") +
  geom_vline(xintercept=c(t-period*18, t+period*18))
...
#         title=element_blank())
# 
# plot_gyro_1_d <- scatterplot(exp1$data, "gyroscope", "one", start=0, end=1, derivs=1, stem=TRUE) +
#   xlim(a, b) +
#   ylim(-0.04, 0.04) +
#   theme(axis.title.x=element_blank(),
#         axis.title.y=element_blank(),
...
#                mutate(diff = lead(one,2)-lag(one,2)),
#              color="blue")
# plot_gyro_2_d <- scatterplot(exp1$data, "gyroscope", "two", start=0, end=1, derivs=1, stem=TRUE) +
#   xlim(a, b) +
#   ylim(-0.042, 0.042) +
#   theme(axis.title.x=element_blank(),
#         axis.title.y=element_blank(),
#         title=element_blank())
# plot_gyro_3_d <- scatterplot(exp1$data, "gyroscope", "three", start=0, end=1, derivs=1, stem=TRUE) +
#   xlim(a, b) +
#   ylim(-0.04, 0.04) +
#   theme(axis.title.x=element_blank(),
#         axis.title.y=element_blank(),
...
# 
# 
# ggarrange(plot_gyro_1, plot_gyro_2, plot_gyro_3, 
#           nrow=1, legend=FALSE)
# ggarrange(plot_gyro_1_d, plot_gyro_2_d, plot_gyro_3_d, 
#           nrow=1, legend=FALSE)

#foobar <- scatterplot(exp1$data, "gyroscope", "one", start=0.4, end=0.45)
plot_gyro_1
```
...
scatterplot(exp1$data, "gyroscope", "two", start=a, end=b, stem=TRUE) +
  geom_point(aes(x=time_predict, y=extremum_two_predict),
             data=events)
```
...
```{r}
model.1.x <- lm(x ~ extremum_one_predict + extremum_two_predict + extremum_three_predict + extremum_pressure_predict + extremum_p_model_predict,
              data=events)
model.1.y <- lm(y ~ extremum_one_predict + extremum_two_predict + extremum_three_predict + extremum_pressure_predict + extremum_p_model_predict,
              data=events)

model.1.r <- lm(r ~ extremum_one_predict + extremum_two_predict + extremum_three_predict + extremum_pressure_predict + extremum_p_model_predict,
              data=events)

model.1.d <- lm(d ~ extremum_one_predict + extremum_two_predict + extremum_three_predict + extremum_pressure_predict + extremum_p_model_predict,
              data=events)

model.1.m <- lm(m ~ extremum_one_predict + extremum_two_predict + extremum_three_predict + extremum_pressure_predict + extremum_p_model_predict,
              data=events)

model.1.theta <- lm(theta ~ extremum_one_predict + extremum_two_predict + extremum_three_predict + extremum_pressure_predict + extremum_p_model_predict,
              data=events)
