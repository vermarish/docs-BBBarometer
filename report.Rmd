...
performance_by_threshold <- performance_by_threshold %>%
  ...
  mutate(prod=PPV*TPR, norm=sqrt(PPV^2+TPR^2))

# Identify the best threshold value
best_threshold <- performance_by_threshold %>% 
  mutate(metric = norm) %>% # CHANGE THIS TO norm OR prod
  arrange(desc(metric)) %>% 
  slice(1) %>% 
  pull(threshold)
...
performance_by_threshold %>%
  pivot_longer(cols = c(PPV, TPR, norm, prod), names_to = "metric") %>%
  ggplot(aes(x=threshold, y=value, color=metric)) +
  geom_line(size=1) +
  geom_point() +
  #scale_y_continuous(limits=c(0,1)) +
  # set 1 as the upper limit
  scale_x_continuous(limits=function(xlims) { pmax(xlims, c(0,1)) }) +
  scale_color_manual(breaks = c("PPV", "TPR", "norm", "prod"), 
                     values = c("#F8766D", "#619CFF", "#BBBBBB", "#999999"),
