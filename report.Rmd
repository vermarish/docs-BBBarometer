...
Experiment two: Do taps in the center vs. the edge differ in pressure deviation?
 TODO
```{r}
exp2_data <- build_experiment(path="data/experiment_2.csv") %>%
...
TODO this whole chunk
```{r}
exp2_data <- build_experiment(path="data/experiment_2.csv")
# TODO 0-0 is the top left of the display.
exp2_data %>% 
  ggplot(aes(x=x, y=y)) +
  geom_point() +
  xlim(0, 1080) + theme(aspect.ratio=2400/1080) + scale_y_reverse(limits=c(2400,0))
...
exp2_data %>%
  mutate(side = ifelse(y > 1800, "center", "bottom")) %>%
  ggplot(aes(x=extremum_one, y=extremum_three, color=side)) +
  geom_point() +
  geom_hline(yintercept=0, color="#aaaaaa") + 
  geom_vline(xintercept=0, color="#aaaaaa") + 
  xlim(-bounds, bounds) + 
  ylim(-bounds, bounds) +
  ggtitle("Experiment 2 is not bad")
```