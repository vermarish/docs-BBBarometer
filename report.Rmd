...
# We will get a dataframe with 4-columns for each entry in an observation,
...
I should double-check that this labels the data correctly
```{r}
types = c(rep("pressure",5), 
          "touch",
          rep("pressure",5))
one = c(1,2,4,7,11,
        100,
        -1,-2,-4,-7,-11)
foobar = data.frame(type=types, one=one)

df_result <- build_df(foobar, width=3)
print(df_result)
```
...
train_pressure_model <- function(train, test) {
  ...
  ## Fit the model
  touch_model <- glm(label ~ X1 + X2 + X3 + X4 + X5 + X6 + X7, 
                     data=train_df,
                     family = "binomial")
