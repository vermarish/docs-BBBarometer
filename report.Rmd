...
touch_confidence <- train_pressure_model(data, width=4)

# Using that p-column, we can try a bunch of different thresholds
performance_by_threshold <- evaluate_thresholds(data, touch_confidence, sets="training")
```
...
```{r} 
touch_confidence %>% ggplot(aes(x=time, y=p)) + geom_line() + geom_hline(yintercept=0.2, colour="grey", linetype=2)
```  
...
```{r include=FALSE}
# Get waveforms (peaks) from a signal at a set of times
...
# TODO mine_features shouldn't be gyroscope-specific. Take in all the data + a
#      vector of times, and mine gyroscope data AND pressure data. 
#      big task tomorrow morning. call it a "cd .."

# In: TIME-SORTED data tibble with ONLY timestamped gyroscope entries
# Ou: feature tibble containing each wave
mine_features <- function(data, times) {
  ...
  pressure <- data %>% filter(type == "pressure") %>% arrange(time)
  delay <- 2
  period <- pressure$time %>% diff %>% mean
  pressure_times <- times + delay*period
  
  pressure_one <- pull_waveforms(signal = pressure %>% select(time=time, data=one),
...
# Given a dataset containing sensor and touch data, create a dataset consisting of features
...
build_df_touch <- function(data, labeled=TRUE) {
  ...
  features <- mine_features(data, times)
...
# For testing
build_df_touch(data, labeled=FALSE)
```
...
pull_pressure_features <- function(result_pair, times) {
  ...
  features <- c()
  for (time in times) {
    # for each time, get 2 pressure values before it and 6 pressure values after it
...
build_experiment <- function(tidbits=NULL, path="data/experiment_1.csv") {
  ...
  threshold_f1 <- performance_by_threshold %>%
    slice_max(F1) %>%
    slice(1) %>%
    pull(threshold)
  threshold_fhalf <- performance_by_threshold %>%
    slice_max(Fhalf) %>%
    slice(1) %>%
    pull(threshold)
  threshold <- mean(c(threshold_f1, threshold_fhalf))
