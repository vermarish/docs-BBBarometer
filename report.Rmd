...
#### 2a: Collect information from each touch from pressure model
...
build_experiment <- function(tidbits=NULL, path="data/experiment_1.csv") {
  ...
  foobar <- full_join(exp_data_labeled, exp_data_unlabeled,
                      by="event_id",
                      na_matches="never")
  
  result <- list()
  result$df_touch = exp_data_labeled
  result$df_touch_predict = exp_data_unlabeled
  return(result)
}
```

Maybe I take the two ataframes
iterate through
assign an event_id to each thing
so they can later be joined on that event_id if desired
