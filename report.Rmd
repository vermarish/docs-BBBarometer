...
plot1 <- performance_by_threshold %>%
  ...
  ggtitle("Precision and recall by varying threshold")
...
plot2 <- ggplot() +
  ...
  scale_fill_manual(values=c("TP"="#00BB44",
                             "FN"="#A0CCA9",
                             "FP"="#EE4444")) +
  geom_col(data=performance_by_threshold,
  ...
  geom_line(data=performance_by_threshold %>%
            ...
            mapping=aes(x=threshold, y=F*(TP+FN), linetype=metric)) +
  scale_linetype_manual(values=c("F1"=1, "F2"=2, "Fhalf"=3)) +
...
plot3 <- performance_by_threshold %>%
  ...
  ggplot(aes(x=TPR, y=threshold, fill=F,
