...
My name's Rishabh, and this project is a showcase for the skills I've learned studying algorithms and statistics. I'm studying to be a data scientist, a software engineer, or something in between. Labels can be hard. 

Anyhow, here's the premise.

## Abstract

My phone has a gyroscope and a barometer, which respectively measure rotational velocity and air pressure. This paper centers around the phenomenon that when I touch the screen, the barometer registers a spike in air pressure. This is likely because the force of my finger causes a volume deflection inside the phone. The gyroscope is susceptible to recording tremors in the hand holding the phone, but if the barometer can indicate *when* touch input occurs, the phone's motion recorded by the gyroscope can indicate *where* on the screen the touch input occurs. 

This could be applied to create a malicious background process which can read a user's PIN code with access only to device sensor data. This report is a work in progress.

#### Skills practiced: Software Development


* Android app, sensors API
* SQL

#### Skills practiced: Data Science



* Data cleaning
* Denoising time-series via low-pass filter
* Time-series windowed peak detection
...
I've made an Android app with a 10-digit passcode interface and a stopwatch. When the stopwatch is running, it captures the following data:
...
#### Observation: Each data tidbit is timestamped, but sensor events and touch events use different clocks.
...
#### I should expect the min/max timestamp to match for all datatypes.

This is visibly false. Note that I have to remove some of the leading sig figs because the difference occurs in less-significant digits and would otherwise be truncated by R's output.

```{r}
accelerometer %>% select(time) %>% mutate(time = time %% 1e+12) %>% summary()
# The gyroscope/pressure timestamps are ~identical to the accelerometer timestamps.
...
So let's calibrate the touch data. To convert from millis to nanos, the equation is `y = mx+b` where `y` is in nanos, `x` is in millis, `m` is the slope between them (should expect 10^6) and `b` is the y-intercept.

To perform the conversion, my app also records two data points at the start and at the stop.


After calculating, we can confirm the slope using the secant between the start and end points.
...
```
That looks better! The touch clock matches the rest of the sensors' clock.

## Examining the data: Can this really work?

### Graphing documentation

Let 0 and 1 represent the beginning and the end of the touch input. I want a function such that, if I want to graph the middle 50% of the data, I can call the function with parameters start=0.25, end=0.75, and it graphs the proportion of data between those two points.
...
Let's look at the gyroscope sensors and the pressure sensor. **Orange vertical lines represent the occurrence of touch input.**
